...
Genbu delegates all authentication to each cloud SDK's default credential chain. No genbu-specific authentication configuration is needed.

**AWS** (default)

The [AWS SDK default credential chain](https://docs.aws.amazon.com/sdkref/latest/guide/standardized-credentials.html) is used:

| Priority | Method | Use case |
|----------|--------|----------|
| 1 | Environment variables (`AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`) | Local dev, CI |
| 2 | Shared config (`~/.aws/credentials`, `~/.aws/config`) | Local dev |
| 3 | ECS task role (`AWS_CONTAINER_CREDENTIALS_*`) | ECS Fargate/EC2 |
| 4 | EC2 instance profile (IMDS) | EC2 |
| 5 | SSO / Web Identity Token | EKS, SSO |
...
[Application Default Credentials (ADC)](https://cloud.google.com/docs/authentication/application-default-credentials) is used:

| Priority | Method | Use case |
|----------|--------|----------|
| 1 | `GOOGLE_APPLICATION_CREDENTIALS` (service account key JSON) | Local dev, CI |
| 2 | `gcloud auth application-default login` | Local dev |
| 3 | GCE metadata server (attached service account) | GCE, GKE |
| 4 | Cloud Run / Cloud Functions service account | Serverless |
| 5 | Workload Identity Federation | Cross-cloud, GitHub Actions |
...
[DefaultAzureCredential](https://learn.microsoft.com/en-us/azure/developer/go/azure-sdk-authentication) is used:

| Priority | Method | Use case |
|----------|--------|----------|
| 1 | Environment variables (`AZURE_CLIENT_ID`, `AZURE_TENANT_ID`, `AZURE_CLIENT_SECRET`) | CI, service principal |
| 2 | Workload Identity | AKS |
| 3 | Managed Identity (System/User Assigned) | Azure VM, App Service, Container Apps |
| 4 | Azure CLI (`az login`) | Local dev |
| 5 | Azure Developer CLI (`azd auth login`) | Local dev |
...
各プロバイダの認証は全て SDK のデフォルトクレデンシャルチェーンに委譲しています。genbu 固有の認証設定は不要です。

**AWS**（デフォルト）

[AWS SDK 標準クレデンシャルチェーン](https://docs.aws.amazon.com/sdkref/latest/guide/standardized-credentials.html)を使用:

| 優先順 | 認証方法 | 用途 |
|---|---|---|
| 1 | 環境変数 (`AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`) | ローカル開発、CI |
| 2 | 共有設定ファイル (`~/.aws/credentials`, `~/.aws/config`) | ローカル開発 |
| 3 | ECS タスクロール (`AWS_CONTAINER_CREDENTIALS_*`) | ECS Fargate/EC2 |
| 4 | EC2 インスタンスプロファイル (IMDS) | EC2 |
| 5 | SSO / Web Identity Token | EKS、SSO連携 |
...
[Application Default Credentials (ADC)](https://cloud.google.com/docs/authentication/application-default-credentials) を使用:

| 優先順 | 認証方法 | 用途 |
|---|---|---|
| 1 | `GOOGLE_APPLICATION_CREDENTIALS` (サービスアカウントキーJSON) | ローカル開発、CI |
| 2 | `gcloud auth application-default login` | ローカル開発 |
| 3 | GCE メタデータサーバー (アタッチされたサービスアカウント) | GCE、GKE |
| 4 | Cloud Run / Cloud Functions サービスアカウント | サーバーレス |
| 5 | Workload Identity Federation | クロスクラウド、GitHub Actions |
...
[DefaultAzureCredential](https://learn.microsoft.com/en-us/azure/developer/go/azure-sdk-authentication) を使用:

| 優先順 | 認証方法 | 用途 |
|---|---|---|
| 1 | 環境変数 (`AZURE_CLIENT_ID`, `AZURE_TENANT_ID`, `AZURE_CLIENT_SECRET`) | CI、サービスプリンシパル |
| 2 | Workload Identity | AKS |
| 3 | マネージドID (System/User Assigned) | Azure VM、App Service、Container Apps |
